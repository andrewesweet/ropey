...
- **Port over the engine.** An abstract refactoring interface with rope as one adapter. Rejected: rope is the reason this project exists rather than a volatile dependency; there is no credible second engine; the port would be shaped by rope's capabilities anyway (a leaky abstraction) and would push tests towards mock engines, which the Testing Decisions explicitly reject.
...
- If rope were ever replaced (no plan to), the rewrite cost is the server's internals. That is accepted, because the alternative is paying a permanent abstraction tax against a hypothetical.
