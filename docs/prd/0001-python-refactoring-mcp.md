...
The coding agent (Claude in Claude Code / OpenCode) can read and navigate Python well. It runs Astral's ty as an LSP for symbol lookup, go-to-definition, and find-references. What it lacks is a safe way to *change* Python structurally. Today, when the agent needs to rename a symbol used across thirty files, move a function to another module and fix every import, extract a block into a helper, or change a function's signature and update all call sites, it does this by hand with text edits. That is slow, and worse, it is silently unreliable: the agent cannot be sure it found every reference, cannot prove a textual match is the *same* binding, and routinely leaves behind broken or half-renamed code. A text editor is worst at exactly the operations that most need cross-file, binding-aware precision.

[rope](https://github.com/python-rope/rope) is a mature open-source library that does this correctly, but it has no agent-facing interface. Its API speaks in byte offsets and `Project` objects, not in the LSP coordinates the agent already holds.
...
The capability is deliberately scoped to manipulation. Navigation stays with ty; the server never duplicates symbol lookup. The division of labour is that ty finds and reads while ropey changes.
...
5a. As a coding agent, I want to optionally assert the Expected Symbol at my Target Position, so that a stale Position (one where the file changed since my LSP answer) fails loudly as a Structured Failure instead of silently refactoring the wrong code.
...
41. As a coding agent, I want to organise imports (sort, dedupe, expand star-imports, and convert relative to absolute) so that a module's imports are tidy and explicit.
...
- **Transaction model.** Every Refactoring tool takes an `apply` flag. Dry Run (`apply: false`) computes the Change Set and writes nothing. Live Run (`apply: true`) applies via rope `project.do`, atomically with respect to other tool calls (the serial-execution boundary). Process death mid-apply can leave a partial Change Set on disk. That is an accepted risk, recovered via git; tool descriptions should recommend a clean working tree before Live Runs. Both modes return identical detail.
- **Concurrency.** The per-root Project is the consistency boundary: Refactorings against it execute serially, with concurrent tool calls queueing behind an in-process lock (a single global lock is acceptable; rope `Project` is not thread-safe and refactorings are sub-second). A call never observes a half-applied state from another call. Two accepted residual risks (ADR 0002): separate server processes against the same repo are not coordinated, and an out-of-band write landing inside the validate→apply window races the call. Both are recovered via git.
- **Blast Radius output.** Every result enumerates every affected resource with its path, Change Kind (`modified | created | moved | deleted`), `old_path` for moves, and a short description. Full modified content is never returned; the agent uses `git diff` after a Live Run. Creations and module→package conversions appear in the Blast Radius.
- **Project / cache / Freshness (ADR 0002).** One rope `Project` is kept alive in-process per root, constructed with `ropefolder=None` so no cache artifact is written into the repo. The Search Scope is the project root tree minus gitignored files (derived from gitignore into rope's `ignored_resources` or equivalent; it falls back to rope defaults when there is no git repo or gitignore); the root is auto-discovered by walking up to the `.git` root (erring wide) and is overridable via an optional `root` parameter. If no `.git` root exists up the tree and no explicit `root` is given, the call is a Structured Failure asking for an explicit `root`. The server never guesses scope (e.g. falling back to CWD), because a wrong guess under-scopes and silently corrupts. Before every Refactoring the server re-establishes Freshness via rope's mtime-based, source-agnostic `validate()`. The implementer must guard against a typo'd root, because `Project(root)` creates the directory if absent. Reversal is git; rope's in-session undo is a non-relied-upon bonus.
- **Uncertainty and failure (ADR 0003).** rope's `unsure` callback is wired so that only certain occurrences are applied and every Uncertain Occurrence is enumerated in the result as a flagged Location, never silently included and never silently dropped. `RefactoringError` (and any refusal) maps to a Structured Failure stating the failed precondition, never a crash or bare traceback. An unparsable file in scope is a Structured Failure naming the file (`ignore_syntax_errors` stays off, because skipping a broken file would silently drop its occurrences).
...
- **Capacity & latency envelope** (order-of-magnitude expectations; validate during Phase 1, revise if measured reality differs):
  - *Cold start*: the first Refactoring on a root pays rope's lazy parse of the modules it touches. Expect roughly hundreds of files/second parsed, so ~1s on a 1k-file project and tens of seconds worst-case on a 100k-file monorepo whose refactor touches everything. This is acceptable: it happens once per root per server lifetime, and MCP calls are not hard-timeout-bound the way HTTP is.
  - *Warm call*: `validate()` is a stat-only tree pass, on the order of 10⁴–10⁵ stats/second, so milliseconds on normal repos and ~1s only at monorepo scale. This number is the Phase 4 trigger: build hooks only if p50 `validate()` exceeds ~250 ms on the repos it is actually used on (per the logged timings below).
  - *Memory*: a warm Project holds parsed ASTs for touched modules, on the order of tens-to-hundreds of MB on large projects. It is bounded by the LRU root cap (ADR 0002).
  - *Result size*: the Blast Radius is never truncated, since full enumeration is the transparency contract, but each entry is one short line, so even a 500-file rename is a few thousand tokens. The no-file-contents rule is what keeps this bounded.
- **Distribution and human documentation.** Install channels are fixed; mechanics stay implementer's discretion. Claude Code: app-native via the official plugin marketplace, where this repo hosts the marketplace manifest and a plugin bundling the MCP server config, and install is `/plugin marketplace add` + `/plugin install`. OpenCode: the native `opencode.json` MCP config block (official mechanism; no plugin needed). Any other MCP client: a generic MCP JSON snippet with a runnable launch command (e.g. `uvx`-style). Human docs are a README covering what ropey is, the ty/ropey division of labour, the safety contract in user terms, and per-host install, kept current as phases land. Package name, registry (PyPI vs git), and launch tooling are the implementer's choice; prefer official mechanisms, falling back to well-regarded community approaches only where no official one exists.
...
**Deep modules implied by the foundation** (the implementer may refine the boundaries; these are the natural seams, and the labels are non-normative sketches, since module names in code must follow the `CONTEXT.md` vocabulary):

- **Location Translator.** LSP Position/Range ⇄ rope offset, including UTF-16↔code-point conversion, and the Expected Symbol check. Pure, with a narrow interface.
- **Scope Resolver.** Root-discovery rules (walk up to `.git`; no-git ⇒ Structured Failure without an explicit root) and gitignore → exclusion derivation (nested gitignores, negation patterns, no-gitignore fallback). Pure policy over a filesystem snapshot, and the highest-stakes correctness logic in the system, kept out of the infrastructure module so it is trivially unit-testable.
- **Project Provider.** Per-root in-process Project cache (`ropefolder=None`, LRU-bounded), re-establishes Freshness via mandatory `validate()` before each operation, with a typo'd-root guard. Humble: it consumes the Scope Resolver's decisions.
...
- **Uncertainty and Failure Mapper.** Collects Uncertain Occurrences via rope's `unsure` hook; maps refusals → Structured Failure in glossary vocabulary. Pure-ish.
- **Tool layer.** Thin MCP adapter (registration plus Claude-optimised descriptions). Shape, naming, and signatures are the implementer's discretion.
...
- **Phase 1 (v1):** Rename (with docstring/comment and whole-hierarchy options); Move (global / method / module); Extract Method; Extract Variable; Inline; Change Signature (add / remove / reorder params + default changes, updating all call sites); Organise Imports (sort, dedupe, expand star-imports, relative→absolute). Each exposes its full useful option surface.
...
- **Phase 4 (contingent):** Claude Code Hooks plus OpenCode equivalent as an optional, degradable cache-invalidation accelerator. Host `PostToolUse` (`Edit | Write | MultiEdit`) pings the server for a targeted `validate(file)`. It must not be a correctness mechanism: hooks see only host-routed edits, and the foundation's mandatory self-validate remains the backstop whether or not a hook fired. It is contingent on evidence: build only if the logged `validate()` timings (see Operability) cross the trigger in the capacity envelope (~250 ms p50 on real repos), since it is a pure accelerator with no capability value. The hook signal is an advisory integration event ("a host edit occurred at *path*"), never a correctness channel.
...
**Implementer's discretion:** unit tests for the Location Translator (UTF-16↔code-point and boundary cases, Expected Symbol check), the Scope Resolver (nested gitignores, negation patterns, no-git fallback), the Blast Radius Reporter (kind mapping), and the Uncertainty and Failure Mapper are encouraged as fast pure-unit coverage but are not separately mandated, since their behaviour is also observable through the mandated integration tests.
...
- **Dogfooding (practice, not gate):** the server is implemented in Python 3 (forced anyway, since rope is imported in-process), so from the walking skeleton onwards the implementing agent should prefer ropey itself for applicable structural edits to ropey's own source, and file an issue for every friction encountered. Acceptance criteria remain the objective tests; dogfooding is the live user-research loop on top.
- The asymmetric Search Scope risk is the single most important correctness property to preserve: under-scoping silently corrupts (missed references), whereas over-scoping *tracked* files only costs time (rope is binding-aware and won't touch unrelated same-named symbols). When in doubt, scope wider, except for gitignored files, which stay excluded because git cannot revert an edit there.
