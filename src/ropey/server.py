...
def _register_rewrite(mcp: FastMCP, rewriter: RewriteRunner) -> None:
    @mcp.tool(
        ...
        description=(
            ...
            "(one per Wildcard), and exact: true narrows to the Wildcard's "
            "literal name. type matches instances of exactly that class; "
            "instance matches instances of its subclasses (not the base "
            "class's own) — to cover a class and its subclasses, run once "
            "with each. A constrained run reports only the sites the "
            "constraint engages (matched or unsure), not every textual "
            "match. Each Match Site reports its certainty: matched "
            "(constraints satisfied) or unsure (a constraint that cannot "
            "be established at that site, common in dynamically typed "
            "code). Unsure sites are surfaced "
            ...
            "Site for over-matching before setting apply=true. A truncated "
            "Match Site list ('showing N of M', unsure sites first) means "
            "the audit is incomplete — tighten the Pattern or Match "
            "Constraints and preview again before a Live Run. Both modes "
            "report the "
